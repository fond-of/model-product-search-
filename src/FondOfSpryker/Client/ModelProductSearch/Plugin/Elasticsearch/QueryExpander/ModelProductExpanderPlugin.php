...
namespace FondOfSpryker\Client\ModelProductSearch\Plugin\Elasticsearch\QueryExpander;
...
class ModelProductExpanderPlugin extends AbstractPlugin implements QueryExpanderPluginInterface
{
    ...
    {
        if (!array_key_exists('model', $requestParameters)) {
            return $searchQuery;
        }
        ...
        $matchQuery = $this->getFactory()
            ->createQueryBuilder()
            ->createMatchQuery()
            ->setField('model', $requestParameters['model']);
